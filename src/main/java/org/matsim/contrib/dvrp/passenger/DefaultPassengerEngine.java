...
public final class DefaultPassengerEngine implements PassengerEngine, PassengerRequestRejectedEventHandler {
    ...
    public boolean handleDeparture(double now, MobsimAgent agent, Id<Link> fromLinkId) {
        ...
        if (reservedPassengerIdsString != null) {
            ...
            String[] paxIdStrings = reservedPassengerIdsString.toString().split(",");
            ...
            //update the request to include reserved seats and passenger Ids
            //toDo can do the update through the requestCreator only
            //this brings about circular dependency between drt and dvrp...had to add the dependency. Don't know what this means
            //other altrnative is to implement my own PassengerRequests interface that extends the existing one

            //We would need to create multiple request ids no? becuase those are used below or when validating
        } else {
